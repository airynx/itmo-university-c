...
#include <stdlib.h>
#include <unistd.h>
#define GENERATOR_OFILEDIR "./object_log_files/6_producer"
#define HANDLER_OFILEDIR "./object_log_files/6_handler"
int main(void)
{
	...
	if (handler_pid == 0)
	{
		execlp(HANDLER_OFILEDIR, HANDLER_OFILEDIR, (char *)NULL);
	...
	{
		char pid_tostr[22];
		snprintf(pid_tostr, sizeof pid_tostr, "%lld", handler_pid);
		execlp(GENERATOR_OFILEDIR, GENERATOR_OFILEDIR, pid_tostr, NULL);
